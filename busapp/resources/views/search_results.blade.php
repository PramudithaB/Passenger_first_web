    ...
    <section class="timetable-section">
        <div class="timetable-container">
            ...
            <p class="section-subtitle">
                Journey from **{{ ucfirst($from ?? 'Any') }}** to **{{ ucfirst($to ?? 'Any') }}**
                {{ $time ? ' (Showing departures from ' . date('h:i A', strtotime($time)) . ' onwards)' : '' }}
            </p>
                    ...
                    @foreach ($availableRoutes as $route)
                        <div class="route-card">
                            <div class="route-header">
                                <div class="route-from-to">{{ ucfirst($from) }} → {{ ucfirst($to) }}</div>
                                <div class="bus-route-name">{{ $route->bus_route ?? 'Route details unavailable' }}</div>
                            </div>

                            <div class="route-time-label">Departure Time from {{ ucfirst($from) }}</div>
                            <div class="route-time">
                                {{-- FIX APPLIED: Check for null/empty string before parsing time --}}
                                @if (!empty($route->{$from}))
                                    {{ \Carbon\Carbon::createFromTimeString($route->{$from})->format('h:i A') }}
                                @else
                                    N/A
                                @endif
                            </div>
                            ...
                            <div class="route-details">
                                <div class="route-info">
                                    <div class="info-item">
                                        ...
                                        <div class="info-value">{{ $route->bus_number ?? 'N/A' }}</div>
                                    </div>
                                    <div class="info-item">
                                        <div class="info-label">Arrival at {{ ucfirst($to) }}</div>
                                        <div class="info-value">
                                            {{-- FIX APPLIED: Check for null/empty string before parsing time --}}
                                            @if (!empty($route->{$to}))
                                                {{ \Carbon\Carbon::createFromTimeString($route->{$to})->format('h:i A') }}
                                            @else
                                                N/A
                                            @endif
                                        </div>
                                    ...
                                    <div class="info-item">
                                        ...
                                        <div class="info-value">{{ $route->bus_type ?? 'N/A' }}</div>
                                    </div>
                                </div>
                            </div>
                            <a href="#" class="book-button">Book Bus {{ $route->bus_number }}</a> 
                        </div>
                    @endforeach
                </div>
            @else
